...
export default {
	...
	loadGlb(params) {
		...
		gltfLoader.load(
			...
			gltf => {
				gltf.scene.traverse(child => {
					...
					if (cm) {
						if (cm instanceof Array) {
							...
							if (glb.indexOf('ZSGY') > 0 || glb.indexOf('CNTXGY') > 0 || glb.indexOf('BXGSGWFY') > 0) {
				...
				if (showLabelType === 1) {
					self.createShuiChuangLabel(object, labelContent, parseFloat(labelScale))
				}
				if (showMonitorType) {
	...
	cloneLoadObj(obj, data) {
		...
		let newObj
		if (obj.children.length && obj.children[obj.children.length - 1].userData.type === 'label3d') {
	...
	loadData({ data }) {
		...
		if (data0.modelType == 'glb') {
			const glb = 'https://eslink-iot.oss-cn-beijing.aliyuncs.com/' + data0.type + '.glb'
			data0.glb = glb
			this.loadGlb(data0)
		} else {
			const obj = 'https://eslink-iot.oss-cn-beijing.aliyuncs.com/' + data0.type + '.obj'
			const mtl = 'https://eslink-iot.oss-cn-beijing.aliyuncs.com/' + data0.type + '.mtl'
			data0.obj = obj
			data0.mtl = mtl
			this.loadObj(data0)
		}
