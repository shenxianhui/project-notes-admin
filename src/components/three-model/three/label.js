...
let frustum, labelLayer, label3DLayer, monintor3DLabelLayer, showLabel, showMonintorLabel
export default {
	initLabelRenderer(container) {
		...
		this.on(events.render, () => {
			if (showLabel || showMonintorLabel) {
				...
				showLabel &&
					label3DLayer.children.forEach(label => {
						label.element.style.display =
							label.userData.type === 'label3d' && frustum.containsPoint(label.position)
								? 'block'
								: 'none'
					})
				...
				this.labelRenderer && this.labelRenderer.setSize(container.clientWidth, container.clientHeight)
				if (!showMonintorLabel) {
					...
					if (this.label3DRenderer.domElement.firstChild.children.length) {
						...
						for (
							let index = 0;
							index < this.label3DRenderer.domElement.firstChild.children.length;
							index++
						) {
							const item = this.label3DRenderer.domElement.firstChild.children[index]
							item.style.display = item.className === 'monintor-label' ? 'none' : 'block'
						}
					}
					// console.trace()
	...
	removeLabelRenderer() {
		...
		while (this.label3DRenderer.domElement.firstChild) {
			this.label3DRenderer.domElement.removeChild(this.label3DRenderer.domElement.firstChild)
		}
	...
	createMonitorLabel(object, labelScale) {
		...
		label.scale.set(label.scale.x * 0.5, label.scale.y * 0.5, label.scale.y * 0.5)
		this.render()
		return label
	},
   ...
   `
	...
	create3DLabel(object, labelDiv, labelScale, layerType) {
		...
		label.userData.face = new THREE.Vector3(0, -1, 0).applyMatrix4(object.matrixWorld).sub(object.position)
	...
	changeLabelContent() {
		...
		this.root.traverse(mesh => {
			if (mesh.userData.name === '压力表') {
				...
				texts[1].innerText = '压力: ' + (Math.random() * 6.0).toFixed(2) + ' MPa'
				texts[2].innerText = '温度: ' + (Math.random() * 35.0).toFixed(2) + ' ℃'
				texts[3].innerText = '流量: ' + (Math.random() * 15.0).toFixed(2) + ' m/s'
			}
		})
		this.changeLabelContentIndex = 0
	},
