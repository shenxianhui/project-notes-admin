...
export default {
	...
	addDirLight({ position = new THREE.Vector3(1, 1, 1).normalize(), color = 0xffffff, intensity = 1 }) {
		const dirLight = new THREE.DirectionalLight(new THREE.Color(color), intensity)
		dirLight.position.copy(new THREE.Vector3(position.x, position.y, position.z))
		this.scene.add(dirLight)
		lights.push(dirLight)
	},
	addPointLight({
		...
		const plight = new THREE.PointLight(new THREE.Color(color), intensity, distance, decay)
