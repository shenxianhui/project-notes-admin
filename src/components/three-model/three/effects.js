...
export default {
	...
	initEffects() {
		...
		this.waterMaterial = new THREE.MeshPhongMaterial({
			...
			shininess: 30,
			specular: new THREE.Color(0.09666666666666667, 0.09666666666666667, 0.09666666666666667),
			transparent: true,
			opacity: 0.8,
		})
	...
	createWaterPlane(params) {
		let geometry = new THREE.PlaneBufferGeometry(params.size.x, params.size.y, params.size.x, params.size.y)
	...
	setDiagram({ info }) {
		...
		if (info.processFlowDiagramName == '苍南天信' || info.processFlowDiagramName === '章山') {
			// debugger;
			this.yUpReset(info)
		}

		if (info.needFloor) this.needFloor = true
		const startPosition = new THREE.Vector3(info.startPositionX, info.startPositionY, info.startPositionZ)
		const startTarget = new THREE.Vector3(info.startTargetX, info.startTargetY, info.startTargetZ)
		const endPosition = new THREE.Vector3(info.endPositionX, info.endPositionY, info.endPositionZ)
		const endTarget = new THREE.Vector3(info.endTargetX, info.endTargetY, info.endTargetZ)
		this.flyTo({
			endPosition,
			startPosition,
			endTarget,
		...
		info.processFlowDiagramModels.forEach(model => {
			...
			data.scale = new THREE.Vector3(parseFloat(model.scaleX), parseFloat(model.scaleY), parseFloat(model.scaleZ))
			...
			if (model.labelContent && model.labelContent.length) {
				...
				model.labelContent.forEach(content => {
					if (content.showDataType) {
						...
						const text = {
							text: content.monitorTypeName + content.monitorData + content.monitorTypeUnit,
							alarm: content.alarm,
						}
	...
	createGuandao({ points, id, radius, inColor, adduvx, segment }) {
		...
		const newPoints = this.lerpPoints(points)
		let texture = new THREE.TextureLoader().load('https://eslink-iot.oss-cn-beijing.aliyuncs.com/widget-water.jpg')
		texture.wrapS = texture.wrapT = THREE.RepeatWrapping
		texture.matrixAutoUpdate = false
		const watermaterial = new THREE.MeshPhongMaterial({
			color: inColor || new THREE.Color(0.0666666701436, 0.82745099067688, 1.0),
			emissive: inColor || new THREE.Color(0.00666666701436, 0.082745099067688, 0.1),
			emissiveMap: texture,
			alphaMap: texture,
			// emissiveIntensity: 1,
			map: texture,
		...
		guandao.userData.length = radius && radius > 9 ? (length / radius) * 5 : length
		...
		guandaoLayer.children.forEach(guandao => {
			...
			if (userData.water) {
				...
				for (var i = 0; i < position.count; i++) {
					...
					let z = zPoints[i] + (Math.cos(x * 0.1 + time) + Math.sin(y * 0.1 + time)) * 0.5
					position.setZ(i, z)
				}
				position.needsUpdate = true
			}
